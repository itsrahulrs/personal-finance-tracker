...
const CategoryScreen = ({ navigation }) => {
    ...
    const updateCategory = async () => {
        ...
        try {
            const token = await AsyncStorage.getItem("authToken");
            const response = await fetch(`http://192.168.31.167:8000/api/category/${selectedCategory.id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token}`,
        ...
        Alert.alert("Confirm", "Are you sure you want to delete this category?", [
            { text: "Cancel", style: "cancel" },
            {
                ...
                onPress: async () => {
                    try {
                        const token = await AsyncStorage.getItem("authToken");
                        const response = await fetch(`http://192.168.31.167:8000/api/category/${categoryId}`, {
                            method: "DELETE",
                            headers: {
                                "Authorization": `Bearer ${token}`,
                            },
                ...
                renderItem={({ item }) => {
                    return (
                        <View style={styles.item}>
                            ...
                            {/* Expanded View for Transactions */}
